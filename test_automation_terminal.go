...
// Helper to resize vt100.Canvas as it doesn't have a direct resize method like pyte
// This is a simplified version. A real resize would re-flow text.
func (c *vt100.Canvas) ResizeTo(newCols, newLines int) {
    c.Lock() // vt100.Canvas has its own mutex
    defer c.Unlock()

    // Create new backing slices
    newChars := make([]vt100.ColorRune, newCols*newLines)
    // Initialize with default colors and blank runes
    for i := 0; i < len(newChars); i++ {
        newChars[i].Fg = vt100.Default
        newChars[i].Bg = vt100.DefaultBackground
        newChars[i].R = ' ' // Initialize with spaces
    }
    
    // Copy old content, simplified: top-left aligned
    oldW, oldH := c.Width(), c.Height()
    for y := uint(0); y < oldH && y < uint(newLines); y++ {
        for x := uint(0); x < oldW && x < uint(newCols); x++ {
            oldIndex := y*oldW + x
            newIndex := y*uint(newCols) + x
            if int(oldIndex) < len(c.Chars()) { // Chars() returns a copy
                 // This is tricky. c.Chars() is a copy. We need to access internal state or use methods.
                 // Let's assume we need to re-implement this part if direct access is not good.
                 // For now, this part of ResizeTo is conceptual.
                 // A proper resize is very complex. NewCanvas already sets up chars.
                 // This custom ResizeTo is more about re-initializing if needed.
            }
        }
    }
    // This is non-trivial. For now, let's assume NewCanvas() sets initial size,
    // and we work with that. If dynamic resize is needed, vt100.Canvas would need more features
    // or we'd manage the grid entirely separately.
    // The Python version uses fixed cols/lines. So this might not be strictly needed.
    // The `vt100.Canvas` `Resize()` method re-creates based on MustTermSize().
    // We want to set a specific size.
    
    // HACK: Re-initialize with new dimensions by reflection or by modifying struct fields if possible
    // This is not ideal. The vt100.Canvas is not designed for easy arbitrary resize after creation
    // without losing content or complex reflowing.
    // For this script, we'll rely on the initial size set in setupPtyAndShell.
    // If vtScreen.W() and vtScreen.H() are not public setters, this is hard.
    // Let's assume this function is a placeholder for a more robust resize if required.
    // The existing vt100.Canvas.Resize() uses MustTermSize().
    // We will just create it with desired dimensions and not resize it later.
    // So, this ResizeTo method is illustrative and won't be directly used if not essential.
    // The canvas is initialized with MustTermSize(), then we call vtScreen.Clear()
    // The `defaultPtyCols`, `defaultPtyLines` are used for vtScreen.ResizeTo in setup.
    // The vt100.Canvas does not have a direct `ResizeTo` method.
    // We'll set Width and Height via a (hypothetical or to-be-added) method or ensure NewCanvas uses our defaults.
    // For now, let's assume vtScreen is created with the correct defaultPtyCols, defaultPtyLines.
    // The `vt100.NewCanvas()` uses `MustTermSize()`. We need to override this.
    // A workaround: create, then immediately try to adjust.
    // The `vt100.Canvas` struct has `w` and `h` (lowercase).
    // The `Resize` method re-makes `c.chars`.
    // Simplest path: use `vt100.Canvas` as is, and if `MustTermSize` is not `defaultPtyCols/Lines`,
    // the screen representation might differ from the shell's idea of its size.
    // For now, this ResizeTo will not be used, assuming initial setup is sufficient.
}
